...
class Sudoku(object):
    ...
    def sort_least_constraining_value(self, assign_loc_row, assign_loc_col, possible_values):
      ...
      for value in possible_values:
        ...
        for i in range(3):
            for j in range(3):
                if value in self.assignment[top_left_row + i][top_left_col + j] and top_left_row + i != assign_loc_row and top_left_col + j != assign_loc_col:
                  num_conflict_current +=1

        value_constraining[value] = num_conflict_current
      
      return sorted(possible_values, key = lambda x: value_constraining[x]) # least constraining will be in the front
    ...
    def solve_sudoku(self):
        ...
        for value in reversed(self.sort_least_constraining_value(empty_loc_row, empty_loc_col, temp)):
